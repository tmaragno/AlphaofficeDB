...
mv mysqlkey.pub /tmp/."
#
ssh -o StrictHostKeyChecking=no -i mysqlkey opc@$mysqlcs_ip sudo su - oracle -c mv /tmp/mysqlkey.pub ~oracle/.ssh/.
...
ssh -o StrictHostKeyChecking=no -i mysqlkey opc@$mysqlcs_ip sudo su - oracle -c rm ~oracle/.ssh/mysqlkey.pub
ssh -o StrictHostKeyChecking=no -i mysqlkey opc@$mysqlcs_ip sudo su - oracle -c cat /etc/sudoers /home/opc/oracleSudo.txt > /tmp/oracleSudo
ssh -o StrictHostKeyChecking=no -i mysqlkey opc@$mysqlcs_ip sudo su - oracle -c cp /tmp/oracleSudo /etc/sudoers
ssh -o StrictHostKeyChecking=no -i mysqlkey opc@$mysqlcs_ip sudo su - oracle -c chown root:root /etc/sudoers
ssh -o StrictHostKeyChecking=no -i mysqlkey opc@$mysqlcs_ip sudo su - oracle -c chmod 440 /etc/sudoers
